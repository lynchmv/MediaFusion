...
class CombinedPlaylistParser:
    ...
    async def run(self):
        ...
        # Fetch existing TV channels from the database for comparison
        ...
        # In-memory set to track unique (clean_name, url) for the current playlist
            ...
            for channel in channels_in_group:
                ...
                if is_event:
                    ...
                    try:
                        ...
                        if not found_dates:
                            raise ValueError(
                                "Date could not be found in the event title"
                            )
                        ...
                        event_genre = channel.attributes.get(
                            "group-title", "Other Sports"
                        ).strip()

                        # Check for existing events in Redis within a time window
                        ...
                        for event_key in existing_event_keys:
                            existing_event_json = await REDIS_ASYNC_CLIENT.get(
                                event_key
                            )
                            if existing_event_json:
                                existing_event = (
                                    MediaFusionEventsMetaData.model_validate_json(
                                        existing_event_json
                                    )
                                )
                                if (
                                    ...
                                    # Check if stream already exists in the existing event
                                    new_stream_url = channel.url
                                    stream_exists = any(
                                        s.url == new_stream_url
                                        for s in existing_event.streams
                                    )

                                    if not stream_exists:
                                        # Add new stream to existing event
                                        ...
                                        await REDIS_ASYNC_CLIENT.set(
                                            event_key,
                                            existing_event.model_dump_json(
                                                exclude_none=True
                                            ),
                                            ex=int(
                                                (
                        ...
                        if not found_duplicate_event:
                            # If no duplicate found, create a new event
                            stream_as_dict = {
                                "meta_id": event_id,
                                "name": raw_name,
                                "url": channel.url,
                    ...
                    clean_name = channel.attributes.get(
                        IPTVAttr.TVG_NAME.value, raw_name
                    )
                    genres = [
                    ...
                    # In-memory deduplication for the current playlist
                    ...
                    if in_memory_key in unique_channels_in_group:
                        ...
                        self.logger.debug(f"Skipped in-memory duplicate TV channel: '{channel_name}' with URL '{channel.url}'.")
                        continue
                    unique_channels_in_group.add(in_memory_key)

                    found_duplicate_tv = False
                    for existing_channel in existing_tv_channels_in_db:
                        self.logger.debug(f"Comparing '{channel_name}' with existing '{existing_channel.title}'. Fuzzy ratio: {fuzz.ratio(channel_name, existing_channel.title)}.")
                        if fuzz.ratio(channel_name, existing_channel.title) >= FUZZY_MATCH_THRESHOLD:
                            found_duplicate_tv = True
                            self.logger.debug(f"Found fuzzy duplicate TV channel '{channel_name}' with existing '{existing_channel.title}'.")
                            # Check if stream already exists in the TVStreams collection
                            new_stream_url = channel.url
                            existing_stream = await models.TVStreams.find_one(
                                models.TVStreams.url == new_stream_url,
                                models.TVStreams.ytId == None,
                                models.TVStreams.externalUrl == None
                            )

                            if not existing_stream:
                                self.merged_tv_streams_count += 1
                                # Add new stream to existing channel
                                new_stream = models.TVStreams(
                                ...
                                # Insert the new stream into the TVStreams collection
                                await new_stream.save()
                                self.logger.info(f"Merged new stream for '{channel_name}' into existing TV channel '{existing_channel.title}'.")
                            else:
                                self.skipped_duplicate_tv_channels_count += 1
                                self.logger.info(f"Skipped duplicate TV channel '{channel_name}' as stream already exists in '{existing_channel.title}'.")
                            break # Found a duplicate, no need to check further
        ...
        if channel_batch:
            self.logger.debug(f"Processing a batch of {len(channel_batch)} TV channels.")
        ...
        if event_list:
            ...
            for event in event_list:
                ...
                if ttl > 0:
                    ...
                    pipeline.zadd("events:all", {event_key: event.event_start_timestamp})
                    for genre in event.genres:
                        pipeline.zadd(f"events:genre:{genre}", {event_key: event.event_start_timestamp})
